...
| **6 · Maps ✓** (phase 5) | `v-map` (+ `-header`/`-filters`/`-pa-search`/`-disclaimer`/`-baselayer-controls`/`-toggler`) | **MapLibre chosen**. Overlay state started as `useMapStore` (Pinia); now `useMapOverlays` provide/inject, scoped to the `Map/Index.vue` tree. |
