...
4. CSS = BEM, NAMESPACE `ct-`.
   ...
   EXCEPTION: a migrated component may keep unprefixed legacy classes ONLY when reusing existing shared Webpacker SCSS mixins as-is and rewriting them is out of scope for that pass. Precedent (do not extend without cause): `ListingPageCard`, Wave 8 `Stats*` (remaining: `Coverage`/`TooltipInfo`), Wave 9 `Dropdown`, Wave 10 `Pame/*` (the `Pame/Table/*` PAME-data-table family, T8 scope — not `Attributes/Pame/*`, a same-named but unrelated PAME-assessments-list family closed this session, see CHANGELOG.md). Default for any component migrated from here on: fresh `ct-`-prefixed styles (`ct-card__date`, not `card__date`) even if markup was copied from a legacy `.vue` file.
