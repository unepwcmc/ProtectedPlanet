    ...
    class Importer
      ...
      def import
        ...
        unless validate_required_views_exist
          ...
          if Wdpa::Portal::Config::StagingConfig.test_mode?
            error_msg += "Check that dummy data generation succeeded."
          else
            error_msg += "Step 1 materialized views may not be ready yet."
          end
        ...
        results[:protected_areas] = import_protected_areas
        ...
        # TO_BE_DELETED_STEP_1: Test mode cleanup - remove once Step 1 materialized views are ready
        # Cleanup dummy data if in test mode
        if Wdpa::Portal::Config::StagingConfig.test_mode?
      ...
      def import_protected_areas
        ...
        # Import attributes
        attribute_result = Wdpa::Portal::Importers::AttributeImporter.import
        
        # Import geometries
        geometry_result = Wdpa::Portal::Importers::GeometryImporter.import(
          Wdpa::Portal::Config::StagingConfig.get_staging_table_name_from_live_table('protected_areas')
        )
        
        {
          attributes: attribute_result,
          geometries: geometry_result
        }
      end

      def import_related_sources
        # TODO_IMPORT: Update these paths once Step 1 provides the actual source data locations
        # The current implementation uses placeholder paths for PARCC and Irreplaceability data
        
        # For now, skip related sources during testing to avoid path issues
        # These can be enabled once Step 1 provides the actual source data
        if Wdpa::Portal::Config::StagingConfig.test_mode?
          Rails.logger.info "Skipping related sources import in test mode"
          return {
            parcc: { success: true, imported_count: 0, errors: [] },
            irreplaceability: { success: true, imported_count: 0, errors: [] }
          }
        end
        
        # Use shared service with EXACT same configuration as GDB
        # Same paths, same fields, same logic
        parcc_result = Wdpa::Shared::RelatedSourceImporter.parcc_import(
          target_table: Wdpa::Portal::Config::StagingConfig.get_staging_table_name_from_live_table('protected_areas')
        )
        
        irreplaceability_result = Wdpa::Shared::RelatedSourceImporter.irreplaceability_import(
          target_table: Wdpa::Portal::Config::StagingConfig.get_staging_table_name_from_live_table('protected_areas')
        )
