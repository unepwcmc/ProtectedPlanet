class Region < ApplicationRecord
  ...
  def protected_areas_per_iucn_category(exclude_oecms: false)
    ...
    correct_order.each do |key, _value|
    ...
    ActiveRecord::Base.connection.execute("
      ...
      INNER JOIN (
        ...
        INNER JOIN countries_protected_areas
          ...
          AND countries_protected_areas.country_id IN (#{countries.pluck(:id).join(',')})
  ...
  def sources_per_region(exclude_oecms: false)
    sources = ActiveRecord::Base.connection.execute("
      ...
      FROM sources
      INNER JOIN countries_protected_areas
      ON countries_protected_areas.country_id IN (#{countries.pluck(:id).join(',')})
