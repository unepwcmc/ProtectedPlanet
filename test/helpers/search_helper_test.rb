...
class SearchHelperTest < ActionView::TestCase
  ...
  test 'clear_filters_link, given params, returns an a tag to remove
   the current search filters' do
    params = ActionController::Parameters.new ({q: 'boneman', country: 123, action: 'yo', controller: 'mama'})
  ...
  test 'clear_filters_link, given params with a main filter, returns an a tag to the main filter' do
    params = ActionController::Parameters.new ({region: 234, country: 123, main: 'region', action: 'yo', controller: 'mama'})
  ...
  test 'clear_filters_link, given params, returns nothing if there are
   no filters' do
    params = ActionController::Parameters.new ({q: 'boneman', action: 'yo', controller: 'mama'})
    assert_equal '', clear_filters_link(params)
  end

  test 'search_title, given the params with a query, returns the search title' do
        params = ActionController::Parameters.new ({q: 'manbone', has_parcc_info: 'true'})
  ...
  test 'search_title, given params with no query, returns the search title' do
    params = ActionController::Parameters.new ({'has_parcc_info' => true, 'main' => 'has_parcc_info'})
  ...
  test 'search_title, given params with a model that doesnt exist, returns the default search title' do
        params = ActionController::Parameters.new ({'country' => 999, 'main' => 'country'})
  ...
  test 'search_title, given params without a main param, returns the default search title' do
        params = ActionController::Parameters.new ({'country' => 123, 'has_parcc_info' => true})
    assert_equal 'Protected Areas', search_title(params)
  end
end
