class ProtectedAreaPresenter
  ...
  def parse_management_plan management_plan
    if (management_plan.is_a? String) && (management_plan.starts_with?("http"))
      ActionController::Base.helpers.link_to(I18n.t('stats.management-plan.button'), management_plan, title: I18n.t('stats.management-plan.button-title', site: @protected_area.name))
    else
      management_plan
    end
  end
