...
import type { SearchAreasProps } from '@/types/backend'
...
const baseProps: SearchAreasProps = {
...
function mountSearchAreas(props: Partial<SearchAreasProps> = {}) {
  return mount(SearchAreas, { props: { ...baseProps, ...props } })
}
...
describe('SearchAreas', () => {
  ...
  it('submits a new search term, resets filters, and writes it to the shared download store', async () => {
    ...
    await wrapper.find('.search--autocomplete input').setValue('yosemite')
    await wrapper.find('.search--autocomplete input').trigger('keyup.enter')
    await vi.waitFor(() => expect(fetch).toHaveBeenCalled())
