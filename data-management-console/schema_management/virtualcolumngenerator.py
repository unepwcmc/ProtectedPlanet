...
class VirtualColumnGenerator:
    ...
    def create_1to1_functions_and_triggers(cursor, fact_table_name, reference_table_name, value_column_name,
                                           target_columns, arg_types, known_as):
...
#        SqlRunner.execute_file_with_substitution(cursor, '../sql/template_staging_1to1.sql', replacement_dict)
    ...
    @staticmethod
    def create_1ton_functions_and_triggers(cursor, fact_table_name, reference_table_name, value_column_name,
                                           source_columns, arg_types, association_table_alias):
        abbreviated_get_function = AbbreviateName.abbreviate_name(
        ...
        for table in schema_tables:
            for fk in table.foreign_keys():
                ...
                if fk.is_one_to_one():  # a 1-1 key
                    ...
                    VirtualColumnGenerator.create_1ton_functions_and_triggers(cursor, table.name,
                                                                              fk.target_table,
                                                                              fk.lookup_columns[0],
                                                                              fk.source_columns,
                                                                              arg_types,
