...
class ProtectedAreaTest < ActiveSupport::TestCase
  ...
  test '.bounds returns the bounding box for the PA geometry' do
    protected_area = FactoryBot.create(:protected_area, the_geom: 'POLYGON ((-1.5 0, 0 1, 1 2, 1 0, -1.5 0))')
  ...
  test '.without_geometry does not select the_geom' do
    ...
    protected_area = FactoryBot.create(:protected_area, the_geom: geometry_wkt)
  ...
  test '.as_indexed_json returns the PA as JSON with nested attributes' do
    ...
    iucn_category = FactoryBot.create(:iucn_category, id: 456, name: 'IA')
    designation = FactoryBot.create(:designation, id: 654, name: 'National')
    governance = FactoryBot.create(:governance, id: 654, name: 'Regional')
  ...
  test '#with_valid_iucn_categories returns PAs with valid IUCN
   categories' do
    ...
    no_iucn_category = FactoryBot.create(:iucn_category, name: 'Cristiano Ronaldo')

    FactoryBot.create(:protected_area, iucn_category: iucn_category_1)
    FactoryBot.create(:protected_area, iucn_category: iucn_category_2)
    FactoryBot.create(:protected_area, iucn_category: no_iucn_category)
  ...
  test '.nearest_protected_areas returns the closest 2 PAs, ordered by
   distance ASC and memoizes the result' do
    pa = FactoryBot.create(:protected_area, the_geom_latitude: 1, the_geom_longitude: 0)

    search_mock = mock.tap { |m| m.expects(:results).returns([FactoryBot.create(:protected_area)]) }
  ...
  test '::most_visited, given a date, returns an array of most visited PAs for the month' do
    pa1 = FactoryBot.create(:protected_area, site_id: 345)
    pa2 = FactoryBot.create(:protected_area, site_id: 123)
  ...
  test '.pas_with_green_list_on_self_only returns only PAs with green_list_status on the PA record' do
    ...
    pa_with_gl = FactoryBot.create(:protected_area, site_id: 901, green_list_status: gl)
    FactoryBot.create(:protected_area, site_id: 902)
  ...
  test '.pas_with_green_list_on_self_or_any_parcel returns PAs with green list on self or any parcel' do
    gl_pa = FactoryBot.create(:green_list_status, gl_status: 'Green Listed')
    gl_parcel = FactoryBot.create(:green_list_status, gl_status: 'Re-Listed')
    pa_self = FactoryBot.create(:protected_area, site_id: 801, green_list_status: gl_pa)
    pa_parcel_only = FactoryBot.create(:protected_area, site_id: 802)
    FactoryBot.create(:protected_area_parcel, site_id: pa_parcel_only.site_id, site_pid: '802_A', green_list_status: gl_parcel)
    pa_none = FactoryBot.create(:protected_area, site_id: 803)
  ...
  test '#pa_or_any_its_parcels_is_greenlisted is true when PA has Green Listed or Relisted' do
    gl = FactoryBot.create(:green_list_status, gl_status: 'Green Listed')
    pa = FactoryBot.create(:protected_area, site_id: 701, green_list_status: gl)
    assert pa.pa_or_any_its_parcels_is_greenlisted

    relisted = FactoryBot.create(:green_list_status, gl_status: 'Re-Listed')
    pa2 = FactoryBot.create(:protected_area, site_id: 702, green_list_status: relisted)
  ...
  test '#pa_or_any_its_parcels_is_greenlisted is true when only a parcel is green listed' do
    gl = FactoryBot.create(:green_list_status, gl_status: 'Green Listed')
    pa = FactoryBot.create(:protected_area, site_id: 703)
    FactoryBot.create(:protected_area_parcel, site_id: pa.site_id, site_pid: '703_A', green_list_status: gl)
    assert pa.pa_or_any_its_parcels_is_greenlisted
  end

  test '#pa_or_any_its_parcels_is_greenlisted is false when PA and parcels have no green list' do
    pa = FactoryBot.create(:protected_area, site_id: 704)
  ...
  test '#pa_or_any_its_parcels_is_greenlist_candidate is true when PA is Candidate' do
    gl = FactoryBot.create(:green_list_status, :candidate)
    pa = FactoryBot.create(:protected_area, site_id: 705, green_list_status: gl)
  ...
  test '#pa_or_any_its_parcels_is_greenlist_candidate is true when only a parcel is Candidate' do
    gl = FactoryBot.create(:green_list_status, :candidate)
    pa = FactoryBot.create(:protected_area, site_id: 706)
    FactoryBot.create(:protected_area_parcel, site_id: pa.site_id, site_pid: '706_A', green_list_status: gl)
    assert pa.pa_or_any_its_parcels_is_greenlist_candidate
  end

  test '#pa_or_any_its_parcels_is_greenlist_candidate is false when PA is Green Listed' do
    gl = FactoryBot.create(:green_list_status, gl_status: 'Green Listed')
    pa = FactoryBot.create(:protected_area, site_id: 707, green_list_status: gl)
