...
- **Name length caution**: Postgres identifiers are limited to 63 characters. Because we prefix live table names during swap (e.g., `bkYYMMDDHHMM_`), **new live table names must leave room for the backup prefix**. Example: `countries_protected_areas` becomes `bk2512051519_countries_protected_areas` during backup.
  - Guideline: keep live table names comfortably under the limit (e.g., \(\le\) 45–50 chars) so backup names, index names, and constraint names still fit.
