...
**Next:** finish Wave 3 (`search-site`, deferred — still on Vue2, still imports the legacy `TabsFake.vue`); rewrite migrated components onto Tailwind (including the Wave 8 "reuse legacy SCSS" exceptions, once a rewrite is deliberately scheduled); amCharts 4→5 (deferred out of Wave 8); Wave 9 (PA show `attributes-*`); Wave 10 (PAME); Webpacker removed last.
