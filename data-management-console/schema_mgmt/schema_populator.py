...
class SchemaPopulator():
    ...
    @staticmethod
    def create_schema(schema_to_populate, cursor):
        ...
        try:
            ...
            SqlRunner.execute(f'../sql/{schema_to_populate}/extra.sql', cursor)
    ...
    def create_source_schema(schema_to_populate):
        ...
        try:
            ...
            Logger.get_logger().info("Creating staging tables")
            app_schema_tables = list(Extractor.get_all_definitions(SOURCE_SCHEMA_FILE,objectid_table_def=objectid_table_def))
            PostgresExecutor.replace_tables(app_schema_tables, 'staging', True, schema_to_populate)

            Logger.get_logger().info("Creating main tables")
            app_schema_tables = list(Extractor.get_all_definitions(SOURCE_SCHEMA_FILE, historical_table_def, ingestion_table_def,objectid_table_def))
            PostgresExecutor.replace_tables(app_schema_tables, None, True, schema_to_populate)
