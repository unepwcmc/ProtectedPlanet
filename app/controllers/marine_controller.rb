class MarineController < ApplicationController
  ...
  #Static stats
  before_action :all_marine_protected_areas, only: [:index]
  before_action :marine_total_pas_count, only: [:index]
  before_action :marine_statistics, only: [:index]
  before_action :maine_protected_areas_growth, only: [:index]
  before_action :marine_stats_items, only: [:index]

  #Calculated stats
  before_action :top_regions_countries_with_most_marine_protected_areas, only: [:index]
  ...
  def all_marine_protected_areas
    @all_marine_protected_areas = Rails.cache.fetch(
      "marine/all_marine_protected_areas/#{marine_data_cache_version}",
      expires_in: 12.days
    ) { ProtectedArea.marine_areas.limit(3) }
  end
