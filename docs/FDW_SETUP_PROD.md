...
**Prerequisites:**
- Ensure you use the `wcmc` user role that Rails is currently using to connect to the DB. (If Rails uses a different role to connect to the DB, then change `wcmc` to that role name)
...
GRANT USAGE ON FOREIGN SERVER portal_srv TO wcmc;

-- Map wcmc role to portal_ro_user on Portal DB
CREATE USER MAPPING IF NOT EXISTS FOR wcmc
  SERVER portal_srv
...
GRANT USAGE ON SCHEMA portal_fdw TO wcmc;
GRANT CREATE ON SCHEMA portal_fdw TO wcmc;

-- Set default privileges for future tables
ALTER DEFAULT PRIVILEGES IN SCHEMA portal_fdw GRANT SELECT ON TABLES TO wcmc;
...
WHERE foreign_table_schema='portal_fdw' 
ORDER BY 1;
```
...
### Materialized views schema (contract)
...
- Grant access to a specific application role (wcmc), not PUBLIC:
...
#### Re-import updated foreign tables on PP DB
