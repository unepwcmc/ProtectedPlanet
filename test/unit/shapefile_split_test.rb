...
class TestShapefileSplit < ActiveSupport::TestCase
  test '.split runs the correct ogr2ogr command to split a geo database
   in to `n` shapefiles and then zips them individually' do
    ...
    Ogr::Shapefile.expects(:system).with("ogr2ogr -overwrite -skipfailures -f \"ESRI Shapefile\" -lco ENCODING=UTF-8 ./poly_0.shp -dialect sqlite -sql \"SELECT * FROM poly LIMIT 100 OFFSET 0\" #{filename}")
    Ogr::Shapefile.expects(:system).with("ogr2ogr -overwrite -skipfailures -f \"ESRI Shapefile\" -lco ENCODING=UTF-8 ./poly_1.shp -dialect sqlite -sql \"SELECT * FROM poly LIMIT 100 OFFSET 100\" #{filename}")

    expected_shapefile_paths = ['./poly_0.shp', './poly_1.shp']

    ogr = Ogr::Split.new
    shapefile_paths = ogr.split filename, layer_name, 2
